...
public class bipartiteGrapgDFS {
// author: Tarun Verma
...
public static boolean dfsBipartite(int node, int n, int vis[]) {
	if(vis[node] == -1) {
		vis[node] = 1;
	}
	for(int i: adj[node]) {
...
public static boolean isBipartite(int n, int vis[]) {
	for(int i =1;i<=n;i++) {
		if(vis[i] == -1) {
			if(!dfsBipartite(i, n, vis)) {
				return false;
