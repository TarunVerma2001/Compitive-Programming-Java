...
public class RatInAMazeSelf {
	...
	public static boolean ratInM(ArrayList<ArrayList<Integer>> arr, int x, int y, int n, ArrayList<ArrayList<Integer>> solArr) {
		if(x == n-1 && y == n-1) {
		...
		if(isSafe(arr, x, y, n)) {
			solArr.get(x).set(y, 1);
			if(ratInM(arr, x+1, y, n, solArr)) {
				return true;
			}
			if(ratInM(arr, x, y+1, n, solArr)) {
				return true;
			}
			solArr.get(x).set(y, 0);	//backtarck
			return false;
		}
		return false;
	...
	public static void main(String[] args) {
		...
		boolean temp1 = ratInM(arr, 0, 0, n, solArr);
		System.out.println(temp1);
		if(ratInM(arr, 0, 0, n, solArr) == true) {	
